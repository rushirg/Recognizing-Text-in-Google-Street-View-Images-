...
#my-comment# xTrain = read_data("train", labelsInfoTrain, imageSize, path)
...
#my-comment# yTrain = map(x -> x[1], labelsInfoTrain[2])

#my-comment# yTrain = convert(Array{Int64,1},yTrain)

tic()
#Convert from character to integer
#my-comment# yTrain = map(Int,yTrain)
...
#Save predictions
writetable("juliaSubmissionRF_Model.csv", labelsInfoTest, separator=',', header=true)

accuracy = nfoldCV_forest(yTrain, xTrain, 0, 100, 5, 1.0);
println("5 fold accuracy: $(mean(accuracy))")	

println("Submission file saved in $(path)/code/juliaSubmissionRF_Model.csv")
