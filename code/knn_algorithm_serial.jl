...
yPredictions =72 
for i in 2:size(xTest, 2)
 nRows = size(xTrain, 1)
 imageI = Array(Float32, nRows)
 for index in 1:nRows
  imageI[index] = xTest[index, i]
 end
 yPredictions = vcat(yPredictions, assign_label(xTrain, yTrain, k, imageI))
end
toc()
...
#Save predictions
writetable("$(path)/code/Output/juliaKNNSerialSubmission.csv", labelsInfoTest, separator=',', header=true)

println("Submission file saved in $(path)/code/Output/juliaKNNSerialSubmission.csv")	
